...
function SEARCH_DEL
{
	...
	$searchstring = Get-AdUser $userid.SamAccountName -DomainController $dc -Properties Manager|select SamAccountName,Manager
	if ($searchstring.Manager -ne $null){
		$found_del = @()
		foreach ($user in $users){
			$found_del = $found_del + $(Get-AdUser $user.SamAccountName -DomainController $dc -Properties Manager|?{($_.Manager -like $searchstring.Manager) -and ($_.SamAccountName -ne $userid.SamAccountName)})
		}
		$TB_LOG.Appendtext("$(Get-Date -format "dd-MM-yyyy HH:mm:ss")`t")
